            ...
            with open(media_file, 'rb') as file:
                if media_file.endswith('.mp4'):
                    ...
                    with BytesIO() as output:
                        try:
                            Image.open(file).save(output, format='PNG')
                            output.seek(0)
...
def sendMediaGroup(images, caption='new message', reply_to_message_id=None):
        ...
        media = l[0]
        files = l[1]
        response = requests.post(mediaGroupURL, data={'chat_id': chat_id, 'media': json.dumps(media), 'reply_to_message_id': reply_to_message_id}, files=files, verify=False)
        sleep(int(timeout))
    ...
    with open(csv_path) as csv_file:
        for line in csv.DictReader(csv_file):
            if line is not None and line['id'] == msg_id and line['affiliate_link'][:3] == 'htt':
                ...
                price, link, folder_path, images_path_list, msg_id = csvLine[2], csvLine[3], csvLine[4], csvLine[5], csvLine[0]
