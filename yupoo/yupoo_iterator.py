...
def setAffiliateLink(next_url, no_link_recived_cnt, link_received_cnt):
    if next_url.startswith(('https://www.aliexpress.com/item/', 'https://www.aliexpress.us/item',
                            'https://www.he.aliexpress.com/item')):
...
main_urls = [f'https://{site_id}.x.yupoo.com/albums?page={i}' for i in range(1, pages_count)]



for index, url in enumerate(main_urls):
    album_urls = get_album_urls()
    destination_folder = f'/Users/user/Desktop/YUPOO/yupoo_{index}'
    csv_path = f'/Users/user/Desktop/YUPOO/Items_{index}.csv'
    create_csv()
